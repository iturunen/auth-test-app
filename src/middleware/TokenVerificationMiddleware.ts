...
// var JWS_REGEX = /^Bearer ([A-Za-z0-9_-]+\.){2}[A-Za-z0-9_-]{1}/;
...
interface JwkKey  {
  ...
  n: string;
  use: string;
  x5c: string[]; // This is an array of strings
  x5t: string;
  //'x5t#S256': string;
}
...
const fetchJwks = async (): Promise<JwkKey[]> => {
...
const verifyToken = async (authHeader:string |undefined): Promise<OperationSuccess> => {
  ...
  const key = jwksKeys.find((k) => k.kid === kid) as JwkKey;
  if (!key) {
...
export const typeraVerifyTokenMiddleware: Middleware.Middleware<{ jwtToken: string }, typeraResponse.Unauthorized<{ message: string }>> = async (request) => {
